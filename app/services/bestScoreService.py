...
from app.schema.bestScoreSchema import BestScoreLeaderboardRes, BestScoreEntry, CurrentUserBestScore
...
class BestScoreService:
    ...
    async def upsert_best_score(self, user_id: str, game_type: str, mode: str, score: int, session_id: str, session=None):
        ...
        if not existing:
            ...
            await self.collection.update_one(
                {"_id": existing["_id"]},
                {"$set": {"best_score": score, "session_id": session_id, "achieved_at": datetime.now(timezone.utc)}},
                session=session,
            )
